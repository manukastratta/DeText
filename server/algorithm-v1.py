...
def getCleanText(raw_training_set):
    ...
    for page_text in raw_training_set:
        cleanTxt = text_from_html(page_text)
        ...
        clean_training_set.append(cleanList)
    return clean_training_set
...
raw_training_set = getRawTextFromTrainingSet()
...
for i in range(0, TRAINING_SET_SIZE+NO_WARNINGS):
    if shouldDisplayContentWarning(clean_training_set[i]) == True:
